...
namespace Entidades.Presentadores
{
    ...
    {        
        ...
        public void IniciarPartida(Usuario jugadorUno, Usuario jugadorDos, string aux, CancellationToken cancelador)
        {
            match = new Partida(DateTime.Now, jugadorUno, jugadorDos, aux);
            tarea = new Task(()=>IniciarPartidaAdmin(cancelador));
            if (Match != null)
            {
        ...
        private void IniciarPartidaAdmin(CancellationToken cancelador)
        {
            match?.EmpezarPartida();            
            while (match?.Ganador == "vacio" && !cancelador.IsCancellationRequested)
            {
                Thread.Sleep(3000);
